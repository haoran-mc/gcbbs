...
import (
	...
	time2 "github.com/haoran-mc/gcbbs/pkg/utils/time"
	"gorm.io/gorm"
)
...
func (s *sCheckin) Store() error {
	uid := s.ctx.Auth().ID // 获取当前用户的id

	var checkin model.Checkins
	f := model.Checkin().M.Where("user_id", uid).Find(&checkin) // 根据id查找用户的签到表
	if f.Error != nil {
		return f.Error
	}
	...
	err := db.DB.Transaction(func(tx *gorm.DB) error {
		...
		c := tx.Model(&model.IntegralLogs{}).Create(&model.IntegralLogs{
			UserId:  uid,
			Rewards: consts.CHECKINReward, // 1
